...
DROP TABLE IF EXISTS `detoperbbva`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `detoperbbva` (
  ...
  `referenciaCom` varchar(20) DEFAULT NULL,
  ...
  `requeridaCargaLlaves` int(1) DEFAULT 0,
  ...
  KEY `sucursal_bbva_pinp` (`sucursal`) USING BTREE,
  KEY `terminal_bbva_pinp` (`terminalsist`) USING BTREE,
  CONSTRAINT `sucursal_bbva_pinp_fk1` FOREIGN KEY (`sucursal`) REFERENCES `sucursales` (`sucursal`) ON UPDATE CASCADE,
  CONSTRAINT `terminal_bbva_pinp_fk1` FOREIGN KEY (`terminalsist`) REFERENCES `terminales` (`terminal`) ON UPDATE CASCADE
